...
class SudokuGrid:
	...
	# function to place a value in certain square. if we are solving it will return true, else will return false.
	def place(self, val):
	...
	# sets the currently selected empty cell to 0.
	def clear(self):
		row, col = self.selected
		if self.gui_squares[row][col].value == 0:
			self.gui_squares[row][col].set_temp(0)
	...
	# a function for deciding when to stop solving, as long as there is a '0' on the board, solving is in progress.
	...
	# our main function for solving the board
	def solve(self):
		find = static_helpers.find_empty(self.model)  # find an empty square to solve on the board, if no square was
		# found, the board is solved.
		if not find:
			return True
		else:  # find_empty returns a square with 0 in it for it to solve.
			row, col = find
